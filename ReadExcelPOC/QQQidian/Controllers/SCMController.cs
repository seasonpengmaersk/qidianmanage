...
namespace QQQidian.Controllers
{
    public class SCMController : Controller
    {
        ...
        private ILogger<SCMController> log_;
        public IHttpHelper httpHelper_ { get; private set; }


        private static string SCM_APP_ID = "202036512";
        private static string SCM_APP_SECRET = "Z3LSN4Xmna9UbU9Z";

        public SCMController(ILogger<SCMController> logger, IHttpHelper helper)
        {
            log_ = logger;
            httpHelper_ = helper;
        }
        ...
        public async Task<IActionResult> getStatistics([FromForm]string queryDatetime)
        {
            ...
            try
            {
                ...
                using (var client = new HttpClient())
                {
                    ...
                    string fileName = String.Format("SCM_json_{0}.csv", queryDatetime);
        ...
        {
            string url = String.Format("https://api.qidian.qq.com/cgi-bin/token?grant_type=client_credential&appid={0}&secret={1}",SCM_APP_ID,SCM_APP_SECRET);
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        else
                        {
                            System.Text.Encoding encoding = new System.Text.UTF8Encoding(true);
                            byte[] byteArray = encoding.GetBytes(csvContent);
                            string fileName = String.Format("SCM_cusInfoJson_{0}.csv", string.Format("{0:yyyyMMddHHmmss}", DateTime.Now));
                            fileName = HttpUtility.UrlEncode(fileName, Encoding.GetEncoding("UTF-8"));
                            var data = Encoding.UTF8.GetPreamble().Concat(byteArray).ToArray();
        ...
        {
            string testFileName = String.Format("SCM_cusInfoJson_{0}", string.Format("{0:yyyyMMdd}", DateTime.Now));
            string yesterdayFileName = String.Format("SCM_cusInfoJson_{0}", string.Format("{0:yyyyMMdd}", DateTime.Now.AddDays(-1)));
        ...
        {
            string testFileName = String.Format("SCM_ownerInfoJson_{0}", string.Format("{0:yyyyMMdd}", DateTime.Now));
            string yesterdayFileName = String.Format("SCM_ownerInfoJson_{0}", string.Format("{0:yyyyMMdd}", DateTime.Now.AddDays(-1)));
            string baseDir = AppContext.BaseDirectory;
            ...
            try
            {
                ...
                else
                {
                    ...
                    else
                    {
                        System.Text.Encoding encoding = new System.Text.UTF8Encoding(true);
                        byte[] byteArray = encoding.GetBytes(csvContent);
                        string fileName = String.Format("SCM_ownerInfoJson_{0}.csv", string.Format("{0:yyyyMMddHHmmss}", DateTime.Now));
                        fileName = HttpUtility.UrlEncode(fileName, Encoding.GetEncoding("UTF-8"));
                        var data = Encoding.UTF8.GetPreamble().Concat(byteArray).ToArray();
